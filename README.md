...
2. 사용 환경·기술 스택·작업 스타일을 인터뷰합니다.
3. 모듈을 하나씩 제안하면서, 각 모듈이 무엇을 하는지·당신에게 맞는지 설명합니다.
4. 당신이 승인한 것만 설치하며, 플레이스홀더는 직접 물어보고 채웁니다. 무조건 복붙하지 않습니다.

## 이게 뭔지

**맞는 말:** 이건 학습형 설치기입니다. 목표는 설치를 끝냈을 때 자신이 선택하고 이해한 구성이 남는 것이지, 복사한 파일 더미가 아닙니다.

**틀린 말:** 원클릭 닷파일 덤프가 아닙니다. "전부 복사" 경로는 의도적으로 없습니다. 모듈을 고르고 플레이스홀더를 채우는 마찰 자체가 결과를 '내 것'으로 만드는 과정입니다.

## 하네스 패턴 한 문단 요약

`CLAUDE.md`는 매 세션 자동으로 로드되므로 최대한 작게 유지해야 합니다. 짧고 항상 적용되는 규칙만 인라인으로 두고, 분량이 크거나 상황에 따라 필요한 규칙은 `rules/` 파일로 분리합니다. `CLAUDE.md`는 각 규칙에 대한 **트리거 라인**만 유지하다가, 해당 상황이 오면 모델이 외부 파일을 온디맨드로 읽습니다. 절대 위반해서는 안 되는 결정론적 가드레일에는 **훅(hooks)**을 붙여, 행동 규칙 하나만이 유일한 방어선이 되지 않도록 합니다. 깊고 다단계인 절차는 **스킬(skills)**로 만들어 둡니다. 결과적으로 세션당 토큰 소모가 줄고, 모델 집중력이 높아지며, 규칙은 필요한 순간에만 로드됩니다.
...
├── INSTALL.md                 # Claude가 따르는 오케스트레이션 지침
├── LICENSE
├── docs/
│   ├── 01-concepts.md         # 라우터 패턴의 이유; 인라인 vs 외부 분리
│   ├── 02-architecture.md     # .claude/ 레이아웃과 각 디렉터리의 역할
│   ├── 03-customization-guide.md  # 의사결정 가이드 — 특정 취향을 강요하지 않음
│   └── 04-references.md       # 참고 자료
├── examples/                  # 플레이스홀더 위주의 예시 (참고용, 복붙 금지)
...
MIT — [LICENSE](LICENSE) 파일 참조. 어트리뷰션 없이 자유롭게 사용·수정·재배포·빌드할 수 있습니다.
